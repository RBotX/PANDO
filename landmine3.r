...
schools2=schools2[schools2[,"Family"] %in% paste0("fam",1:29),]
...
for(fam in unique(schools2[,"Family"])){
  ...
  testidx1 = sample(which( (schools2[,"Family"]==fam) & (schools2[,"Label"]==1) ),floor(length(which((schools2[,"Family"]==fam) & (schools2[,"Label"]==1)))*0.85) )
  testidx2 = sample(which( (schools2[,"Family"]==fam) & (schools2[,"Label"]==-1) ),floor(length(which((schools2[,"Family"]==fam) & (schools2[,"Label"]==1)))*0.85) )
...
cat("starting pando\n")
mshared=TunePando(TrainMultiTaskClassificationGradBoost,train,val,fitTreeCoef="nocoef",fitLeafCoef="ridge",trainrate = 0.01,cviter=200,cv=3,cvrate=0.1)
cat("starting pando2\n")
mshared2=TunePando(TrainMultiTaskClassificationGradBoost2,train,val,fitTreeCoef="ridge",fitLeafCoef="nocoef",trainrate = 0.01,cviter=200,cv=3,cvrate=0.1)
...
mbinary=TunePando(vanillaboost2,binaryData,binaryVal,fitTreeCoef="nocoef",fitLeafCoef="nocoef",trainrate=0.01,cviter=200,cv=3,cvrate=0.1)
