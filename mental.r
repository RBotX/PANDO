...
load("/home/dan/Downloads/MentalDisorders.read.rda")
...
colnames(data)=c(colnames(data)[1:(ncol(data)-2)],"Label","Family")
...
for(l in 1:NUM_TESTS){
  ...
  for(fam in unique(train[,"Family"])){
    ...
    logitModels[[toString(fam)]]= cv.glmnet(x=as.matrix(tr[,-which(colnames(tr) %in% c("Family","Label"))]),y=tr[,"Label"],family="binomial",alpha=1,maxit=10000,nfolds=5, thresh=1E-4)
  }
  ...
  mlogitbinary = cv.glmnet(x=as.matrix(rbind(binaryData,binaryVal)[,-which(colnames(tr) %in% c("Family","Label"))]),y=rbind(binaryData,binaryVal)[,"Label"],family="binomial",alpha=1,maxit=10000,nfolds=5, thresh=1E-4)
  ...
  mgplasso = cv.grpreg(gplassoX, gplassoy, group=gplassotraindata$groups, nfolds=5, maxit=10000,seed=777,family="binomial",trace=TRUE)
