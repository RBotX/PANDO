...
GenerateNonLinearData=function(ntasks=5,d=50,ntrain=1000,ntest=300,seed=777){
  ...
  #qs[1:ntasks]=runif(ntasks,0.4,0.5)
  allmatrix=c()
  groups=c()
  ...
  for(t in 1:ntasks){
    ...
    per = rnorm(n=d,mean=0,sd=0.02) ## perturbation per task, intuitively: the more we perturb, the less "in common" the tasks have
...
maxdepths=c(3,5,7,9,11,30)
mshared=TunePando(TrainMultiTaskClassificationGradBoost3,trdata=train,valdata=val,fitCoef="obo",trainrate=0.1)
...
for(fam in unique(train[,"Family"])){
  ...
  m0=TunePando(vanillaboost,tr,tr.val,maxdepths=maxdepths,cv=2)
...
binaryVal = val
binaryVal["Family"]="1"
mbinary=TunePando(vanillaboost,binaryData,binaryVal,trainrate=0.01)
...
test = data$data[data$testidx,]
val = data$data[data$validx,]
mshared=TunePando(TrainMultiTaskClassificationGradBoost3,trdata=train,valdata=val,fitCoef="obo",maxdepths=maxdepths)
perTaskModels=list()
logitModels=list()
for(fam in unique(train[,"Family"])){
  ...
  m0=TunePando(vanillaboost,tr,tr.val,maxdepths=maxdepths)
  perTaskModels[[toString(fam)]]=m0
  logitModels[[toString(fam)]]= cv.glmnet(x=as.matrix(tr[,-which(colnames(tr) %in% c("Family","Label"))]),y=tr[,"Label"],family="binomial",alpha=1,maxit=10000,nfolds=4, thresh=1E-4)
}
...
binaryVal = val
binaryVal["Family"]="1"
mbinary=TunePando(vanillaboost,binaryData,binaryVal,maxdepths = maxdepths)
mlogitbinary = cv.glmnet(x=as.matrix(binaryData[,-which(colnames(tr) %in% c("Family","Label"))]),y=binaryData[,"Label"],family="binomial",alpha=1,maxit=10000,nfolds=4, thresh=1E-4)
