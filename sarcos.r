...
set.seed(1)
sarcos_train = read.csv("sarcos_train.csv")
test = read.csv("sarcos_test.csv")
samplesPerTask = sum(sarcos_train[,"Family"]=="fam1") ### 
idxPerTask = sample(1:samplesPerTask,10000)
alltests=c()
NUM_TESTS=1
for(l in 1:NUM_TESTS){
  ...
  set.seed(l)
  train=c()
  for(i in 1:7) ## sarcos has 7 tasks
  {
    train = rbind(train,sarcos_train[(samplesPerTask*(i-1))+idxPerTask,]) ## we take the same samples per task
  }
  train = train[sample(nrow(train)),] ## shuffle
  validx = sample(1:nrow(train),nrow(train)*0.2)
  val = train[validx,]
  train = train[-validx,]
  ...
  mbinary=TrainMultiTaskClassificationGradBoost(binaryData,iter=iter,v=rate,groups=matrix(1,nrow=nrow(binaryData),ncol=1),controls=rpart.control(),ridge.lambda=ridge.lambda,target="regression",val=binaryVal)  
