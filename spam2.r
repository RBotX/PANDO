...
if(perTaskMethods){
  ...
  for(fam in unique(train[,"Family"])){
    ...
    m0=TunePando(vanillaboost2,tr,tr.val,trainrate = 0.01,cviter=200,maxdepths=c(3,5,6,7),cps=c(0),cv=-1,cvrate=0.01)
    # m0 = TrainMultiTaskClassificationGradBoost(tr,valdata=tr.val,groups = matrix(fam,nrow=nrow(tr),ncol=1),iter=iter,v=0.01,
    #                                            controls=rpart.control(), ridge.lambda = ridge.lambda,target="binary")  
...
allpreds=read.csv("spamAllPreds.csv")
allpreds=allpreds[,-which(colnames(allpreds)=="X")]
methods=colnames(allpreds)
families=unique(test[,"Family"])
for(method in c("PANDO","PANDO2","BB","BinaryLogit","GL")){
  aucs=0
  for(fam in families){
...
for(fam in families){
  ...
  rc1=pROC::roc(as.factor(allpreds[idxs,"Label"]),as.numeric(allpreds[idxs,"PANDO"]))
